...
class GroqAgent(EnvironmentInteractingAgent):
    ...
    def step(self, goal) -> AgentInteractionResult:
        ...
        print(f"Executed action: {action_for_execution}")
        ...
        with open("file.json", "w") as f:
            json.dump(action_for_execution, f, indent=2)
    ...
    def create_prompt(self, goal: str, ui_elements: str) -> list:
        ...
        if self.prev_actions:
            prev_actions_str = "\n".join(
                [f"- {self.action_to_string(a)}" for a in self.prev_actions[-10:]]
            )
        else:
        ...
        messages = [
            {
                ...
                "content": "\n".join([
                    ...
                    "You will be shown the goal, previous actions, and current UI.",
                    "",
                    "CRITICAL: Output ONLY ONE action at a time - the immediate next action needed.",
                    "Do NOT output multiple actions. Do NOT plan ahead.",
                    "You will be called again after each action completes.",
                    "",
                    "IMPORTANT: When the goal is fully accomplished, output: {\"action_type\": \"done\"}",
                    "Check the current UI and previous actions to verify goal completion.",
                    "",
                    "## Available Actions:",
                    "- click: {\"action_type\": \"click\", \"element_id\": \"ui_X\"}",
                    "- long_press: {\"action_type\": \"long_press\", \"element_id\": \"ui_X\"}",
                    "- scroll: {\"action_type\": \"scroll\", \"direction\": \"up|down|left|right\"}",
                    "- input_text: {\"action_type\": \"input_text\", \"text\": \"your text\"}",
                    "- open_app: {\"action_type\": \"open_app\", \"app_name\": \"App Name\"}",
                    "- navigate_home: {\"action_type\": \"navigate_home\"}",
                    "- navigate_back: {\"action_type\": \"navigate_back\"}",
                    "- wait: {\"action_type\": \"wait\"}",
                    "- done: {\"action_type\": \"done\"}: USE THIS WHEN GOAL IS COMPLETE",
                    "",
                    "## UI Tree Format:",
                    "- [element_id] element_type (properties)",
                    "- Use element_id to interact with elements",
                    "- Only click [clickable] elements",
                    "",
                    "Output format: Single JSON object only. No arrays, no multiple objects.",
                ]),
            },
            {
                "role": "user",
                "content": "\n".join([
                    ...
                    ui_elements,
                    "",
                    "What is the NEXT SINGLE action? (JSON object only)",
                ]),
            },
        ]
    ...
    def convert_id_to_coords(self, action: dict, id_to_bounds: Dict[str, str]) -> dict:
        """Convert element_id to coordinates and sanitize action format for AndroidWorld"""

        action_type = action.get("action_type")

        # Click and long_press: convert element_id to coordinates
        ...
        # Scroll: only keep direction
        elif action_type == "scroll":
            return {
                "action_type": "scroll",
                "direction": action.get(
                    "direction", "down"
                ),  # default to down if missing
            }

        # Input text: only keep text
        elif action_type == "input_text":
            return {"action_type": "input_text", "text": action.get("text", "")}

        # Open app: only keep app_name
        elif action_type == "open_app":
            return {"action_type": "open_app", "app_name": action.get("app_name", "")}

        # Simple actions: only keep action_type
        elif action_type in ["navigate_home", "navigate_back", "wait", "done"]:
            return {"action_type": action_type}

        # Unknown action type: return as-is (let AndroidWorld handle the error)
        else:
